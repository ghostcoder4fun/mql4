...
int OnInit()
{
   ...
   PrintFormat("[INIT] EA initialized for %s,M1 | TRADE_AMOUNT_USD=%.2f | LEVERAGE=%d",
...
double ComputeLots(double entryPrice)
{
   ...
   if(lots < LotsMin)
   {
      PrintFormat("[COMPUTE_LOTS] Computed lots too small: %.5f", lots);
      return(0.0);
   }
   ...
   PrintFormat("[COMPUTE_LOTS] Computed lots=%.2f for price=%.5f", normLots, entryPrice);
   return(normLots);
}
...
void OnTick()
{
   ...
   {
      PrintFormat("[SKIP] Tick ignored for symbol %s", Symbol());
      return;
   }
   ...
   if(have_future_preds)
   {
      ...
      // Detailed logging why trade might not enter
      if(!(allAgree && rollingAcc >= 50.0 && ArraySize(openTrades) == 0 && correct == 1.0))
      {
         PrintFormat("[SKIP_TRADE] allAgree=%s rollingAcc=%.2f openTrades=%d correct=%.2f",
                     allAgree ? "true":"false", rollingAcc, ArraySize(openTrades), correct);
      }
